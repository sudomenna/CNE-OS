...
5. **Passo 5 — reclassificar contato** — `MOD-CONTACT.reclassify(contactId)` ([`BR-CONTACT-CLASSIFICATION`](../50-business-rules/BR-CONTACT-CLASSIFICATION.md)): recalcula com base nas transações remanescentes não-reembolsadas (hierarquia `mentorado > student > customer > lead`); INSERT `contact_status_history`; `TE-CONTACT-CLASSIFICATION-CHANGED` se mudou.
