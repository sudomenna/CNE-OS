...
- Arquivos que POSSUI (edita):
  - `lib/db/schema/refund.ts` (`refund`, `refund_effect_log`)
...
- Arquivos que LÊ (read-only):
  - `docs/30-contracts/01-enums.md` (`refund_status` — criar em tarefa serial; OQ-REFUND-01)
...
| `reason` | text | não | — | — |
| `status` | text | não | `requested` | CHECK `IN ('requested','approved','rejected','processed','cancelled')` — enum a criar |
...
- `OQ-REFUND-01` — `refund_status` precisa virar enum real em `30-contracts/01-enums.md`. Hoje tabela usa CHECK texto. Tarefa serial.
