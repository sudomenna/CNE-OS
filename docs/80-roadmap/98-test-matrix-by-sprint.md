...
| **15 — RBAC + Integrations** | `rbac/grant-permission.test.ts` (grant/revoke gera audit_log; CannotModifyAdminRole; erros tipados), `rbac/list-role-matrix.test.ts` (matriz retorna roles + permissions + assignments), `channel/create-channel-account.test.ts` (duplicata → DuplicateChannelAccountError; credentials encriptadas antes de persistir), `channel/update-channel-account.test.ts` (ChannelAccountNotFoundError; isActive toggle), `db/crypto.test.ts` (encrypt/decrypt roundtrip; tampering detectado) | `grantPermissionAction`/`revokePermissionAction` persiste via DB real + gera audit_log; `getRoleMatrixAction` retorna matriz completa; `createChannelAccountAction` guard integration.configure (401 sem permissão); `updateChannelAccountAction` credenciais re-encriptadas; idempotência duplicate-guard | `tests/e2e/settings-permissions.spec.ts` (CT-PERM-01..07): admin vê card Permissões → abre matriz → checkboxes Admin disabled → toggle non-admin persiste → non-admin vê alerta read-only; `tests/e2e/settings-integrations.spec.ts` (CT-INT-01..11): hub com 5 cards → WhatsApp clicável → form write-only → validação client → Brevo placeholder → Digital Guru read-only → non-admin UNAUTHORIZED | config manual de WhatsApp via UI sem redeploy; confirmar que webhook entrante encontra channel_account |
