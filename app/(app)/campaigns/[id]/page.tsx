  ...
  return (
    <div className="space-y-8">
      ...
      <nav aria-label="Breadcrumb" className="flex items-center gap-2 text-sm text-slate-500">
        ...
        <span className="text-slate-900 font-medium">{campaignRow.name}</span>
      </nav>
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900">{campaignRow.name}</h1>
            <Badge
          ...
          <div className="mt-1 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-slate-500">
            <span>
              Marca:{' '}
              <strong className="font-medium text-slate-700">{campaignRow.brandName}</strong>
            </span>
            <span>
              Funil:{' '}
              <strong className="font-medium text-slate-700">{campaignRow.funnelName}</strong>
            </span>
            <span className="font-mono text-xs text-slate-400">{campaignRow.slug}</span>
          </div>
      ...
      <section aria-labelledby="creatives-heading">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 id="creatives-heading" className="text-lg font-semibold text-slate-900">
              Criativos
            </h2>
        ...
        {creatives.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 py-10 text-center">
            <p className="text-sm text-slate-500">Nenhum criativo cadastrado</p>
            <div className="mt-3">
              <CreativeForm campaignId={campaignRow.id}>
                <Button type="button" variant="outline" size="sm">
                  Novo Criativo
              ...
              <tbody className="divide-y divide-slate-100">
                {creatives.map((cr) => (
                  <tr key={cr.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-900">{cr.name}</td>
      ...
      <section aria-labelledby="links-heading">
        ...
        {links.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-slate-300 py-10 text-center">
            <p className="text-sm text-slate-500">Nenhum link rastreável gerado</p>
            <div className="mt-3">
              <TrackableLinkForm
                brandId={campaignRow.brandId}
                brandSlug={campaignRow.brandSlug}
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between">
                    <div className="flex flex-col gap-0.5">
                      <p className="text-xs font-semibold text-slate-600">URL de destino</p>
                      <a
                        href={lk.destinationUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-sm text-slate-700 hover:underline break-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-900 rounded"
                      >
                        {lk.destinationUrl}
                      </a>
                    </div>
                    <div className="flex flex-col items-end gap-0.5 shrink-0">
                      <p className="text-xs font-semibold text-slate-600">Short URL</p>
                  ...
                  {linkedCreative && (
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-500">Criativo:</span>
                      <Badge variant="secondary" className="text-xs">
                        {linkedCreative.name}
